...
function demoBoard(d:Demo, shared=false) {
  return Array.from({length:9},(_,number) => d.lines.includes(number)
    ...
    : `<button data-demo-line="${number}" aria-label="Draw sample line ${number+1}" ${shared && d.you !== d.turn ? "disabled" : ""}></button>`).join("");
}
function demoContent(d:Demo, sharedId?:string) {
  ...
  return `<section class="demo-game"><p class="kicker">Sample room · ${sharedId || "APRON6"}</p><h1>Make a Square</h1>
    ...
    <p class="hint">Demo moves and random demo seat tokens stay in isolated demo storage. Nothing is copied to a real room.</p><p class="hint">In real rooms, we store your nickname, game moves, room code, and a random seat token. Your browser stores the token so you can return to your seat.</p>${share}</aside></div></section>`;
}
function demo() {
  stop(); meta("Demo — Kitchen Table", "Try a two-player sample game that stays in demo storage.", "/demo"); const d=readDemo(); localStorage.setItem(demoKey,JSON.stringify(d));
  app.innerHTML=shell(demoContent(d),true);
  finish();
  document.querySelector("#reset-demo")!.addEventListener("click",()=>{localStorage.removeItem(demoKey);demo();toast("Sample game reset.");});
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-demo-line]").forEach(b=>b.onclick=()=>{const state=readDemo();state.lines.push(Number(b.dataset.demoLine));state.turn=state.turn===0?1:0;localStorage.setItem(demoKey,JSON.stringify(state));demo();document.querySelector<HTMLElement>(".turn-card")?.focus();});
}

async function sharedDemo(id:string) {
  ...
  try {
    ...
    renderSharedDemo(id,room,token);
  } catch(error) {
    app.innerHTML=shell(`<section class="error-state"><p class="kicker">Sample room ended</p><h1>Make a new sample room</h1><p>${esc((error as Error).message)}</p><a class="primary button" href="/demo" data-link>Open the sample game</a></section>`,true);finish();
  }
}
function renderSharedDemo(id:string,d:Demo,token:string|null) {
  app.innerHTML=shell(demoContent(d,id),true); finish();
...
async function roomPage(code:string) {
  stop();meta(`Room ${code} — Kitchen Table`,"Open a Kitchen Table room.",`/room/${code}`);app.innerHTML=shell(`<section class="room-loading"><div class="spinner"></div><h1>Finding room ${esc(code)}</h1><p>The room should be ready in a moment.</p></section>`);finish();
  try {renderRoom(await getRoom(code));} catch(ex) {app.innerHTML=shell(`<section class="error-state"><p class="kicker">Room not found</p><h1>We couldn’t find that room</h1><p>${esc((ex as Error).message)}</p><a class="primary button" href="/#games">Choose a game</a></section>`);finish();}
}
...
function route(navigation=false){busy=false;const path=location.pathname;const shared=path.match(/^\/demo\/([A-Za-z0-9]{6})$/);if(shared)void sharedDemo(shared[1].toUpperCase());else if(path==="/demo"||new URLSearchParams(location.search).get("demo")==="1")demo();else if(path==="/")landing();else if(path==="/privacy"||path==="/terms")legal(path.slice(1) as "privacy"|"terms");else {const room=path.match(/^\/room\/([A-Za-z0-9]{6})$/);room?roomPage(room[1].toUpperCase()):notFound();}if(navigation)focusRoute();}
